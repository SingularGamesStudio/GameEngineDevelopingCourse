...
namespace GameEngine
{
    namespace Render::HAL
    {
        ...
        Math::Matrix4x4f getZRotationMatrix(float angle) {
            Math::Matrix4x4f res = Math::Matrix4x4f::Identity();
            res.SetElement(cos(-angle), 0, 0);
            res.SetElement(-sin(-angle), 1, 0);
            res.SetElement(sin(-angle), 0, 1);
            res.SetElement(cos(-angle), 1, 1);
            return res;
        }
        ...
        void D3D12RHIPrivate::Update(Mesh::Ptr mesh, Material::Ptr material)
        {
            ...
            world = world * getZRotationMatrix(time.count());// rotate around z-axis
            world.SetElement(0.3 + sin(time.count()), 3, 0);// moving along x-axis
